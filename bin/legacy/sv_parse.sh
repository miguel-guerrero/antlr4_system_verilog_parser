...
dir=`dirname $0`
export CLASSPATH="${dir}/java:${dir}/bin/antlr-4.11.1-complete.jar:${CLASSPATH}"
    ...
    if [ "$1" == "-python" ]; then
        ...
        if [ $? == 0 ]; then
            if [ $pyth_binding == 0 ]; then
                ...
                if [ $ext == xml ]; then
                    java Testjson $1.post | $dir/bin/json2xml_filter.py > $inpBase.$ext
                else
                ...
                if [ $ext == xml ]; then
                    ${dir}/python/TestSvVisitor.py $1.post /dev/stdout | \
                        ${dir}/bin/json2xml_filter.py > $inpBase.xml
                else
